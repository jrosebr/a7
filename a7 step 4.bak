...
(define value-of-cps
  (lambda (expr env k)
    (match expr
      ...
      ;extend-env
      ;(lambda (y k^) (if (zero? y) (apply-k k^ k) (apply-env env (sub1 y) k^))) k)
      (`(catch ,body) 
       (value-of-cps body (extend-env-cps k env k) k))
      (`(pitch ,k-exp ,v-exp) (value-of-cps k-exp env (lambda (k^)
                                                        (value-of-cps v-exp env (lambda (v)
                                                                                  (apply-k k^ v))))))
      ;extend-env
      ;(lambda (y k^) (if (zero? y) (apply-k k^ a) (apply-env env (sub1 y) k^))) k)
      (`(let ,e ,body) (value-of-cps e env (lambda (a)
                                             (value-of-cps body (extend-env-cps a env k) k))))
